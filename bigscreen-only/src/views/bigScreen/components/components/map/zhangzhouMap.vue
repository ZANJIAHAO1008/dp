<template>
  <div>
    ...
    <!-- <div class="zz-total" v-if="xzqhObj && title == '关注人员'">
      <div>{{ xzqhObj ? xzqhObj.xzqh : '漳州市' }}</div>
      <ul>
        <li v-for="item in gzryList" :key="item.title">
        ...
        <div class="labels-list">
          <div class="msgList">
            ...
            <co-tooltip placement="topLeft">
              <template slot="title">
                <span>{{ getSafeProperty(personInfo, 'djgzryZwm') }}</span>
              </template>
              <span class="value">{{ getSafeProperty(personInfo, 'djgzryZwm') }}</span>
            </co-tooltip>
          </div>
          <div class="msgList">
          ...
          <div class="msgList">
            ...
            <co-tooltip placement="topLeft">
              <template slot="title">
                <span>{{ getSafeProperty(personInfo, 'djgzrySfzh') }}</span>
              </template>
              <span class="value">{{ getSafeProperty(personInfo, 'djgzrySfzh') }}</span>
            </co-tooltip>
          </div>
          <div class="msgList">
            ...
            <co-tooltip placement="topLeft">
              <template slot="title">
                <span>{{ getSafeProperty(personInfo, 'djgzryZyskxyqk') }}</span>
              </template>
              <span class="value">{{ getSafeProperty(personInfo, 'djgzryZyskxyqk') }}</span>
            </co-tooltip>
          </div>
          <div class="msgList">
            ...
            <co-tooltip placement="topLeft">
              <template slot="title">
                <span>{{ getSafeProperty(personInfo, 'djgzryDjsx') }}</span>
              </template>
              <span class="value">{{ getSafeProperty(personInfo, 'djgzryDjsx') }}</span>
            </co-tooltip>
          </div>
          <div class="msgList">
          ...
          <div class="msgList">
            ...
            <co-tooltip placement="topLeft">
              <template slot="title">
                <span>{{ getSafeProperty(personInfo, 'djgzryZzd') }}</span>
              </template>
              <span class="value">{{ getSafeProperty(personInfo, 'djgzryZzd') }}</span>
            </co-tooltip>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      console.log(this.list, 'this.list mounted');
      this.initChart();
    });
  },
  watch: {
    list: {
      handler(val) {
        if (val.length) {
          console.log(val, 'this.list handler');
          this.initChart();
        }
      },
      deep: true,
  ...
  methods: {
    // 安全地获取personInfo的属性
    getSafeProperty(obj, prop) {
      return obj && obj[prop] ? obj[prop] : '';
    },
    routeToDa(personInfo) {
      const sfzh = this.getSafeProperty(personInfo, 'djgzrySfzh');
      window.open(`http://172.27.2.28:9999/#/ryda?zjhm=${sfzh}`);
    },
    clearChart() {
      if (this.chart) {
    ...
    initChart() {
      ...
      echarts.registerMap('zhangzhou', this.zhangzhou);
      let option = {
        tooltip: {
          trigger: 'item',
          formatter: function (params) {
            // 如果有姓名信息，显示姓名，否则不显示tooltip
            if (params.data && params.data.djgzryZwm) {
              return '姓名: ' + params.data.djgzryZwm;
            }
      ...
      if (this.title == '关注人员') {
        option.series.push({
          ...
          label: {
            ...
            padding: 4,
            formatter(value) {
              return `{name|${value.data.name}}\n关注人员：{gzry|${value.data.zrs}}\n学生群体：{gzry|${value.data.xss}}\n重点人员：{gzry|${value.data.xss}}`;
            },
            rich: {
              name: {
                fontWeight: 'blod',
      ...
      //     currentIndex = (currentIndex + 1) % this.list.length;
      //   }
